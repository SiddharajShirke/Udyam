...
export function EvaluatorStatCard({ label, value, detail, icon: Icon, tone }: EvaluatorStatCardProps) {
  return (
    <div className="group border border-gov-border-light bg-white p-5 shadow-gov-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-brand-orange/50 hover:shadow-gov-md">
      <div className="flex items-start justify-between gap-3"><div className={`flex h-9 w-9 items-center justify-center ${tones[tone]}`}><Icon className="h-4 w-4" aria-hidden="true" /></div><ArrowUpRight className="h-4 w-4 text-gov-text-muted transition-colors group-hover:text-brand-orange" aria-hidden="true" /></div>
